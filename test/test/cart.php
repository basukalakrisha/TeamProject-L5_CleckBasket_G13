...
// Clear buy_now session to prevent interference
...
if (!oci_execute($stmt_get_cart)) {
    ...
    $_SESSION['cart_error'] = "Error fetching cart.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['remove_item']) && isset($_POST['product_id_to_remove'])) {
        ...
        if (oci_execute($stmt_remove, OCI_NO_AUTO_COMMIT)) {
            oci_commit($conn);
            unset($_SESSION['cart'][$product_id_to_remove]);
            $_SESSION['cart_success'] = "Item removed from cart.";
        } else {
            $error = oci_error($stmt_remove);
            error_log("Error removing item: " . $error['message']);
    ...
    if (isset($_POST['update_quantity']) && isset($_POST['product_id']) && isset($_POST['quantity'])) {
        ...
        error_log("Updating quantity: product_id=$product_id, quantity=$quantity");
        ...
        if (!oci_execute($stmt_stock)) {
            $error = oci_error($stmt_stock);
            error_log("Error checking stock: " . $error['message']);
            $_SESSION['cart_error'] = "Error checking product stock.";
        ...
        if ($stock_row && $quantity <= $stock_row['STOCK']) {
            if ($quantity > 0) {
                ...
                if (oci_execute($stmt_update, OCI_NO_AUTO_COMMIT)) {
                    ...
                    error_log("Quantity updated: product_id=$product_id, quantity=$quantity");
                } else {
                    $error = oci_error($stmt_update);
                    error_log("Error updating quantity: " . $error['message']);
...
if ($cart_id) {
    ...
    if (oci_execute($stmt_cart_items)) {
        ...
        $error = oci_error($stmt_cart_items);
        error_log("Error fetching cart items: " . $error['message']);
        $_SESSION['cart_error'] = "Error fetching cart items.";
    }
